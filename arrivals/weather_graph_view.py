...
class WeatherGraphView(Observer, SampleBase):
    ...
    def should_draw_chevron(self, x, yy):
        return (abs(self.viewmodel.sunrises_x[0] - x) <= 1 or abs(self.viewmodel.sunsets_x[0] - x) <= 1 or
            abs(self.viewmodel.sunrises_x[1] - x) <= 1 or abs(self.viewmodel.sunsets_x[1] - x) <= 1) and (
