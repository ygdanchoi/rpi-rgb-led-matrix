...
class WeatherGraphView(Observer, SampleBase):
    ...
    def update(self):
        ...
        for i, point in enumerate(points):
            ...
            color = [255, 127, 127] if self.is_lightning(point) and should_flash else point.color
        ...
        for i, point in enumerate(points[2:27:4]):
            ...
            if max_pop != '0%':
                max_snow = max([pt.snow for pt in points[(p_i - 1):(p_i + 3)]])
                self.draw_text(
    ...
    def draw_stripe_pixel(self, xx, yy, color):
        ...
        if self.viewmodel.get_gol_safe(yy, xx) < -64:
            ...
            self.offscreen_canvas.SetPixel(
                ...
                yy,
                max(
                    color[0] // 2 + self.viewmodel.get_gol_safe(yy, xx) * 9,
                    color[0] // stripe_divisor
                ),
                max(
                ...
                max(
                    color[2] // 2 + self.viewmodel.get_gol_safe(yy, xx) * 3,
                    color[2] // stripe_divisor
                )
            )
